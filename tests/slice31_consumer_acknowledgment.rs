...
fn valid_consumer_acknowledgment_and_closure_publish() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice31_consumer_ack_ok")?;
    let handoff_dir = make_prepared_handoff_workspace(&root)?;
    let output_dir = root.join("ack_workspace");

    let acknowledgment = build_consumer_acknowledgment_receipt(&handoff_dir)?;
    publish_consumer_acknowledgment(&output_dir, &acknowledgment)?;
...
#[test]
fn tampered_handoff_package_fails_closed() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice31_consumer_ack_tamper")?;
    let handoff_dir = make_prepared_handoff_workspace(&root)?;
    fs::write(handoff_dir.join("handoff_package/extra.txt"), b"tamper")?;

    let error = build_consumer_acknowledgment_receipt(&handoff_dir)
        .expect_err("tampered bounded package must fail closed");
    assert!(
        error.to_string().contains("member set mismatch"),
