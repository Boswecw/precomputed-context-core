...
fn valid_lineage_bundle_intake_builds_and_publishes() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice26_lineage_bundle_intake_ok")?;
    let source_bundle_dir = make_prepared_lineage_bundle(&root)?;
    let intake_dir = root.join("intake_workspace");
...
#[test]
fn missing_envelope_fails_closed() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice26_lineage_bundle_intake_missing_envelope")?;
    let source_bundle_dir = make_prepared_lineage_bundle(&root)?;
    fs::remove_file(source_bundle_dir.join("lineage_bundle_envelope.json"))?;

    let error = build_lineage_bundle_intake_receipt(&source_bundle_dir)
        .expect_err("missing envelope must fail closed");
    assert!(error.to_string().contains("envelope") || error.to_string().contains("missing"));
    Ok(())
}
...
fn make_prepared_lineage_bundle(root: &PathBuf) -> Result<PathBuf, Box<dyn Error>> {
    ...
    let bundle_workspace = root.join("bundle_workspace");
