...
fn valid_sealed_release_bundle_builds_and_publishes() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice35_sealed_release_ok")?;
    let attestation_dir = make_prepared_attestation_workspace(&root)?;
    let output_dir = root.join("sealed_workspace");
    ...
    publish_sealed_release_bundle(&attestation_dir, &output_dir, &manifest, &receipt)?;
...
#[test]
fn handoff_boundary_member_drift_fails_closed() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice35_sealed_release_tamper")?;
    let attestation_dir = make_prepared_attestation_workspace(&root)?;
    fs::write(
        attestation_dir.join("handoff_boundary_package/extra.txt"),
        b"tamper",
    )?;

    let error = build_terminal_boundary_manifest(&attestation_dir)
        .expect_err("member drift must fail closed");
    assert!(
        error
            .to_string()
            .contains("handoff boundary package member set mismatch"),
