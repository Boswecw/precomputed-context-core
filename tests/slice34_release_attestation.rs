...
#[test]
fn valid_release_attestation_builds_and_publishes() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice34_release_attestation_ok")?;
    let (release_dir, readiness_dir) = make_prepared_readiness_workspace(&root)?;
    let output_dir = root.join("attestation_workspace");

    let receipt = build_release_attestation_receipt(&release_dir, &readiness_dir)?;
    publish_release_attestation_package(&release_dir, &readiness_dir, &output_dir, &receipt)?;

    assert!(output_dir.join("release_attestation_receipt.json").exists());
    assert!(
        output_dir
            .join("handoff_boundary_package/operator_release_summary.json")
            .exists()
    );
    Ok(())
}
...
#[test]
fn readiness_receipt_continuity_tamper_fails_closed() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice34_release_attestation_tamper")?;
    let (release_dir, readiness_dir) = make_prepared_readiness_workspace(&root)?;

    let readiness_receipt_path = readiness_dir.join("release_readiness_receipt.json");
    ...
    let error = build_release_attestation_receipt(&release_dir, &readiness_dir)
        .expect_err("continuity tamper must fail closed");
    assert!(
        error
            .to_string()
            .contains("release readiness receipt continuity mismatch"),
...
fn make_prepared_consumption_workspace(
    ...
    fs::write(&zip_path, b"slice34 release attestation package bytes")?;
