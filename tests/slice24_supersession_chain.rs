...
use precomputed_context_core::supersession_chain::build_supersession_chain_receipt;
...
fn valid_supersession_chain_builds_complete_lineage() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice24_supersession_ok")?;
    let prepared = make_prepared_supersession_surface(&root)?;

    let receipt = build_supersession_chain_receipt(
        &prepared.promotion_receipt_path,
        &prepared.rollback_receipt_path,
        &prepared.repromotion_receipt_path,
    )?;
...
#[test]
fn repromotion_rollback_hash_mismatch_fails_closed() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice24_supersession_bad_hash")?;
    let prepared = make_prepared_supersession_surface(&root)?;

    let bad_repromotion_path = root.join("bad_re_promotion_receipt.json");
    let mut value: serde_json::Value =
        serde_json::from_slice(&fs::read(&prepared.repromotion_receipt_path)?)?;
    value["rollback_receipt_sha256"] = serde_json::Value::String("0".repeat(64));
    fs::write(&bad_repromotion_path, serde_json::to_vec_pretty(&value)?)?;

    let error = build_supersession_chain_receipt(
        &prepared.promotion_receipt_path,
        &prepared.rollback_receipt_path,
        &bad_repromotion_path,
    )
...
struct PreparedSupersessionSurface {
...
fn make_prepared_supersession_surface(root: &PathBuf) -> Result<PreparedSupersessionSurface, Box<dyn Error>> {
    ...
    Ok(PreparedSupersessionSurface {
