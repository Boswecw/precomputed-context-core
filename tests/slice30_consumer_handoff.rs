...
fn valid_consumer_handoff_builds_and_publishes() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice30_consumer_handoff_ok")?;
    let (activation_dir, consumption_dir) = make_prepared_consumption_workspace(&root)?;
    let output_dir = root.join("handoff_workspace");

    let receipt = build_bounded_consumer_handoff_receipt(&activation_dir, &consumption_dir)?;
...
#[test]
fn missing_attestation_receipt_fails_closed() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice30_consumer_handoff_missing_attestation")?;
    let (activation_dir, consumption_dir) = make_prepared_consumption_workspace(&root)?;
    fs::remove_file(consumption_dir.join("attestation_receipt.json"))?;

    let error = build_bounded_consumer_handoff_receipt(&activation_dir, &consumption_dir)
        .expect_err("missing attestation receipt must fail closed");
    let message = error.to_string();
    assert!(
        message.contains("attestation receipt missing") || message.contains("No such file"),
