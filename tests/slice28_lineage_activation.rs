...
fn valid_lineage_activation_builds_and_publishes() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice28_lineage_activation_ok")?;
    let rehydrate_dir = make_prepared_rehydrate_workspace(&root)?;
    let activation_dir = root.join("activation_workspace");

    let receipt = build_lineage_activation_receipt(&rehydrate_dir)?;
...
#[test]
fn missing_repromotion_receipt_fails_closed() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice28_lineage_activation_missing_repromotion")?;
    let rehydrate_dir = make_prepared_rehydrate_workspace(&root)?;
    fs::remove_file(rehydrate_dir.join("lineage_state/re_promotion_receipt.json"))?;

    let error = build_lineage_activation_receipt(&rehydrate_dir)
        .expect_err("missing re-promotion receipt must fail closed");
    let message = error.to_string();
    assert!(
        message.contains("No such file")
            || message.contains("not found")
            || message.contains("re_promotion_receipt"),
