...
use precomputed_context_core::lineage_bundle::{
    ...
    publish_lineage_bundle, verify_lineage_bundle, LineageBundleSourcePaths,
};
...
#[test]
fn valid_lineage_bundle_publishes_and_verifies() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice25_lineage_bundle_ok")?;
    let sources = make_prepared_lineage_sources(&root)?;
    let bundle_dir = root.join("lineage_bundle");

    let (_manifest, _envelope) = publish_lineage_bundle(&bundle_dir, &sources)?;
    verify_lineage_bundle(&bundle_dir)?;

    assert!(default_lineage_bundle_manifest_path(&bundle_dir).exists());
    assert!(default_lineage_bundle_envelope_path(&bundle_dir).exists());
    Ok(())
}

#[test]
fn extra_member_fails_closed() -> Result<(), Box<dyn Error>> {
    let root = unique_temp_dir("slice25_lineage_bundle_extra")?;
    let sources = make_prepared_lineage_sources(&root)?;
    let bundle_dir = root.join("lineage_bundle");

    let (_manifest, _envelope) = publish_lineage_bundle(&bundle_dir, &sources)?;
    fs::write(bundle_dir.join("rogue.txt"), b"rogue")?;

    let error = verify_lineage_bundle(&bundle_dir).expect_err("extra member must fail closed");
    assert!(error.to_string().contains("member set mismatch"));
    Ok(())
}
