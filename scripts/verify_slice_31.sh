...
ack_hash_1="$(sha256sum target/proof_artifacts/slice31_consumer_acknowledgment/consumer_acknowledgment_report.json | awk '{print $1}')"
cargo run --bin proof_consumer_acknowledgment >/dev/null
ack_hash_2="$(sha256sum target/proof_artifacts/slice31_consumer_acknowledgment/consumer_acknowledgment_report.json | awk '{print $1}')"
if [[ "$ack_hash_1" != "$ack_hash_2" ]]; then
  echo "consumer acknowledgment report hash changed across repeated emission"
  exit 1
fi

test -f target/proof_artifacts/slice31_consumer_acknowledgment/current/consumer_acknowledgment_receipt.json
test -f target/proof_artifacts/slice31_consumer_acknowledgment/current/return_channel_closure_receipt.json

echo "Slice 31 verification passed"
